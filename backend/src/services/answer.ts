...
export class AnswerService {
    ...
    async getNewestAnswerRef(): Promise<NewestAnswerRef> {
        const res = await this.knex('todaysLanguage')
            .max('id', { as: 'id' })
            .first();
        if (!res) {
            throw new Error(
                'Could not find the newest answer ref',
            );
        }
        return res;
    }
